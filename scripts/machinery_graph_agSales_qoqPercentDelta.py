...
de_data[2] = pd.to_numeric(de_data[4], errors='coerce')
cnh_data[2] = pd.to_numeric(cnh_data[4], errors='coerce')
agco_data[2] = pd.to_numeric(agco_data[4], errors='coerce')
...
de_changes = calculate_qoq_change(de_data[2])
cnh_changes = calculate_qoq_change(cnh_data[2])
agco_changes = calculate_qoq_change(agco_data[2])
...
# Save the plot
output_path = os.path.join(parent_dir, 'scratch_work', 'sales_qoq_changes.png')
